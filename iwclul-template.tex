%
% File iwclul-template.tex
%
%% Based on the style files for ACL-2018, with some changes, which were, in turn,
...
\title{Instructions for IWCLUL Proceedings}
...
\begin{abstract}
  ...
  manuscript for the proceedings of IWCLUL. The document itself
...
This document has been adapted from the instructions of ACL
...
to IWCLUL or accepted for publication in its proceedings. All
...
By uncommenting {\verb|\aclfinalcopy|} at the top of this document, it will
compile to produce an example of the camera-ready formatting; by leaving it
commented out, the document will be anonymized for initial submission.  When you
first create your submission on softconf, please fill in your submitted paper ID
where {\verb|***|} appears in the {\verb|\def\aclpaperid{***}|} definition at
the top.
...
(accepted) version.  The IWCLUL \LaTeX\ style will create a titlebox space of
6.35 cm for you when {\verb|\aclfinalcopy|} is commented out.

\subsection{The Ruler}

The IWCLUL 2018 style defines a printed ruler which should be present in the
...
preparing a document without the provided style files, please arrange for an
...
The camera ready copy should not contain a ruler. (\LaTeX\ users may uncomment
the {\verb|\aclfinalcopy|} command in the document preamble.)
...
\LaTeX\ using the \textit{xelatex} command. If your version of
...
For reasons of uniformity, GNU \textbf{Free fonts} should be
...
\end{verbatim}
\end{quote}
in the preamble. If Free Serif is unavailable, use \textbf{Computer
  Modern Roman} (\LaTeX2e{}'s default).  Note that the latter is about
  10\% less dense than Free Serif font.
...
\textbf{Abstract}: Type the abstract at the beginning of the first column. The
width of the abstract text should be smaller than the width of the columns for
the text in the body of the paper by about 0.6 cm on each side. Center the word
\textbf{Abstract} above the body of the abstract using the font size and style
shown in Table~\ref{font-table}. The abstract should be a concise summary of the
general thesis and conclusions of the paper. It should be no longer than 200
words. The font size of the abstract text should be as shown in
Table~\ref{font-table}.
...
The papers may consist of up to eight (8) pages of
...
ACL also encourages the submission of supplementary material
